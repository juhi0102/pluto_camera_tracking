...
class DroneController:
    ...
    def track_blue(self, frame):
        ...
        kp = 0.2
        kd = 0.1  # You may need to adjust this value based on your system's dynamics
        ...
        if M["m00"] != 0:
            ...
            derivative_x = offset_x - prev_error_x
            derivative_y = offset_y - prev_error_y

            # Control the drone based on centroid position with PD control
            if abs(offset_x) > 10:  # Adjust drone's roll
                roll_adjustment = int(offset_x * kp + derivative_x * kd)
                self.drone.rcRoll = 1500 + roll_adjustment
                print("Adjusting roll:", self.drone.rcRoll)
            else:  # Centered in X direction
                self.drone.rcRoll = 1500

            if abs(offset_y) > 10:  # Adjust drone's pitch
                pitch_adjustment = int(offset_y * kp + derivative_y * kd)
                self.drone.rcPitch = 1500 + pitch_adjustment
                print("Adjusting pitch:", self.drone.rcPitch)
            else:  # Centered in Y direction
                self.drone.rcPitch = 1500

            # Update previous errors
            prev_error_x = offset_x
            prev_error_y = offset_y
